 ...
 *
 * With hacks specific to the Field-Object Tumblr theme
 * to go forward onto next page, go back to last page.
	...
	function calc_index(offset, opts) {
		...
		if (index == null) {
			...
			if (index == null) {
				...
				// BEGIN Field-Object hacks
				if (offset > 0 && ++index >= $.jknav.items[opts.name].length) {
					turnPage = 1;
					log('Going forward past last post');
					index = 0;
					}
				else if (offset < 0 && top == Math.floor($($.jknav.items[opts.name][index]).offset().top) && --index < 0) {
					turnPage = 2;
					log('Going back past first post');
					index = $.jknav.items[opts.name].length - 1;
					}
				}
				// END Field-Object hacks

			}
		else {
			if (!opts.circular && (index == 0 && offset == -1))
				return index;
			else if (!opts.circular && (index == $.jknav.items[opts.name].length - 1 && offset == 1))
				return index;
			index += offset;
	...
	function keyup(e, opts) {
		...
		if (ch == opts.up.toLowerCase() || ch == opts.down.toLowerCase()) {
			...
			if (turnPage == 1) {
				...
				if (prev) {
					prev += '#jknav-back-target';
					window.location = prev;
					}
				else {
					log('No last page to load');
					turnPage = 0;
					}
				}
			else {
